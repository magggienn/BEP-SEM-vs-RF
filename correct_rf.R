    ...
    for(ss in 1:4){
      for(m in 1:2){
        for(s in 1:2){
          ...
          rf_model <- randomForest(x = train_latentvars_data_rf, y = train_data_rf$y, tuneGrid = tuneGrid)
          ...
          # store results
          pe.iter = c(sqrt(mean((Y[idx2,] - yhat.sem)^2)), sqrt(mean((test_data_rf$y - yhat_rf)^2)))
...
# save results
save(PE1, file = "simstudy1final_correctrf.Rdata")
...
ggsave("/Users/magggien/Documents/BEP-SEM-vs-RF/sim1_correctrf_100rep.pdf", 
